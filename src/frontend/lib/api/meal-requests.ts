...
export async function getMealRequests(params?: Record<string, string>): Promise<MealRequest[]> {
  try {
    ...
    const url = `/api/request/requests${queryString ? `?${queryString}` : ''}`;
...
export async function getMealRequestStats(): Promise<MealRequestStats> {
  try {
    const response = await fetch('/api/request/requests/stats', {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
...
export async function updateMealRequestStatus(
  ...
  try {
    ...
    const response = await fetch(
      `/api/request/requests/${mealRequestId}/status?${params.toString()}`,
      {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
