...
@router.post("/permissions", response_model=PagePermissionResponse, status_code=status.HTTP_201_CREATED)
async def grant_permission(
    ...
    try:
        permission = await service.grant_permission(session,
...
async def list_permissions(
    ...
    service = PagePermissionService()
    permissions, total = await service.list_permissions(session,
        page=page,
        per_page=per_page,
        role_id=role_id,
        page_id=page_id,
...
@router.post("/email-roles", response_model=EmailRoleResponse, status_code=status.HTTP_201_CREATED)
...
async def add_email(
    ...
    try:
        email = await service.add_email(session,
...
async def list_emails(
    ...
    service = EmailService()
    emails, total = await service.list_emails(session,
        page=page,
        per_page=per_page,
        role_id=role_id,
    )
...
async def mark_user_as_manual(
    ...
    try:
        ...
        # Log the action
        log_service = LogPermissionService()
...
@router.post("/users/{user_id}/override-status", response_model=UserStatusOverrideResponse)
async def override_user_status(
    ...
    try:
        ...
        if request_data.status_override and not request_data.override_reason:
            raise ValidationError("override_reason is required when enabling status override")

        # Only allow override for HRIS users
        if user.user_source != 'hris':
            raise ValidationError(f"Status override is only supported for HRIS users (current source: {user.user_source})")
        ...
        await log_service.log_permission(
            ...
            details={
                ...
                "override_reason": request_data.override_reason if request_data.status_override else None
            }
        )
