...
router = APIRouter(prefix="/meal-requests", tags=["meal-requests"])


# Meal Type Endpoints
@router.post("/types", response_model=MealTypeResponse, status_code=status.HTTP_201_CREATED)
async def create_meal_type(
    meal_type_create: MealTypeCreate, session: AsyncSession = Depends(get_session),
):
    """Create a new meal type."""
    service = MealTypeService()
    try:
        meal_type = await service.create_meal_type(session,
...
async def list_meal_types(
    ...
    """List all meal types."""
    service = MealTypeService()
    meal_types, total = await service.list_meal_types(session, page=page, per_page=per_page)
    return meal_types
...
async def update_meal_type(
    ...
    service = MealTypeService()
    try:
        meal_type = await service.update_meal_type(session,
            type_id=type_id,
...
async def delete_meal_type(
    type_id: int, session: AsyncSession = Depends(get_session),
):
    """Delete a meal type."""
    service = MealTypeService()
    try:
...
@router.post("/statuses", response_model=MealRequestStatusResponse, status_code=status.HTTP_201_CREATED)
async def create_meal_request_status(
    status_create: MealRequestStatusCreate,
    session: AsyncSession = Depends(get_session),
):
    """Create a new meal request status."""
    service = MealRequestStatusService()
    try:
        status_obj = await service.create_meal_request_status(session,
...
async def get_meal_request_status(
    status_id: int, session: AsyncSession = Depends(get_session),
):
...
async def list_meal_request_statuses(
    ...
    service = MealRequestStatusService()
    statuses, total = await service.list_meal_request_statuses(session,
...
async def create_meal_request(
    request_create: MealRequestCreate, session: AsyncSession = Depends(get_session),
):
    """Create a new meal request."""
    service = MealRequestService()
    try:
        meal_request = await service.create_request(session,
...
async def list_meal_requests(
    ...
    service = MealRequestService()
    requests, total = await service.list_requests(session,
        page=page,
        per_page=per_page,
...
async def update_meal_request(
    ...
    try:
        ...
        if hasattr(request_update, "status_id") and request_update.status_id:
            meal_request = await service.update_request_status(session,
...
@router.post("/{request_id}/lines", response_model=MealRequestLineResponse, status_code=status.HTTP_201_CREATED)
...
async def update_meal_request_line(
    ...
    service = MealRequestService()
    try:
        line = await service.update_line(session,
            line_id=line_id,
