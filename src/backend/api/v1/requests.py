...
log_meal_request_line_service = LogMealRequestLineService()
log_meal_request_service = LogMealRequestService()
...
def send_notification_background(
    ...
    if settings.CELERY_ENABLED:
        try:
            ...
            logger.info(
                f"Email task dispatched to Celery for request #{request_id}"
            )
            return
    ...
    try:
        ...
        subject = (
            f"Meal Request Submitted #{request_id} - Confirmation Pending"
        )
    ...
    try:
        ...
        for line in request_lines:
            try:
                ...
                if not employee:
                    logger.error(f"Employee {line.employee_id} not found, skipping line")
                    continue
                ...
                created_line = (
                    await meal_request_service.create_meal_request_line(
...
async def create_meal_request_endpoint(
    ...
    maria_session: AsyncSession = Depends(get_session),
    ...
    try:
        ...
        logger.info(
            f"Creating {len(request_lines)} request lines immediately"
        )
        ...
        if created_lines:
            logger.info(
                f"Successfully created {len(created_lines)} request lines"
            )
        else:
        ...
        if settings.CELERY_ENABLED:
            try:
                from tasks.attendance import fetch_attendance_for_meal_request_task

                fetch_attendance_for_meal_request_task.delay(
                    meal_request_id=meal_request_record.id
                )
                logger.info(
...
async def get_meal_request_stats(
    ...
    logger.info(f"Fetching meal request statistics with filters: requester={requester}, from_date={from_date}, to_date={to_date}")
    try:
...
async def get_meal_request_line(
    ...
    try:
        ...
        if not meal_request_line:
            logger.info(
                f"No meal request lines found for request_id={request_id}"
            )
...
async def update_meal_request_line_status_endpoint(
    ...
    try:
        ...
        meal_request_line = (
            await meal_request_service.update_meal_request_line(
                session,
...
async def update_meal_request_status_endpoint(
    ...
    try:
        ...
        action_type = (
            "approve"
            if status_id == 2
            else "reject" if status_id == 3 else "update_status"
        )
        await log_meal_request_service.log_meal_request(
            session=session,
        ...
        if status_id == 2:  # Approved status
            ...
            logger.info(
                f"Set all lines to accepted for meal request {meal_request_id}"
            )
            ...
            logger.info(
                f"Set all lines to rejected for meal request {meal_request_id}"
            )
...
@router.get(
    "/audit-request", response_model=Optional[List[AuditRecordResponse]]
)
...
@router.put(
    "/update-meal-request", response_model=MealRequestStatusUpdateResponse
)
@limiter.limit("100/minute")
async def update_meal_request_frontend(
    ...
    try:
        ...
        return MealRequestStatusUpdateResponse(
            ...
            closed_by_id=(
                str(meal_request.closed_by_id)
                if meal_request.closed_by_id
                else None
            ),
...
async def soft_delete_meal_request(
    ...
    try:
        ...
        logger.warning(
            f"Request not found for soft delete: request_id={request_id}"
        )
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail=str(e),
