...
async def get_meal_types_paginated(
    ...
    meal_types, total = await service.list_meal_types(
        session, page=page, per_page=limit, active_only=active_only if active_only is not None else False
    )

    # Calculate counts for active/inactive
    all_meal_types, _ = await service.list_meal_types(session, page=1, per_page=1000, active_only=False)
    active_count = sum(1 for mt in all_meal_types if mt.is_active and not mt.is_deleted)
    inactive_count = sum(1 for mt in all_meal_types if not mt.is_active and not mt.is_deleted)
...
async def create_meal_type(
    ...
    """
    service = MealTypeService()
    log_service = LogConfigurationService()
