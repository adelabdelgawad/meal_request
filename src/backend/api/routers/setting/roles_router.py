...
# Define API router
router = APIRouter(prefix="/setting/permissions", tags=["setting-permissions"])
active_directory = LDAPAuthenticator()

# Initialize services
user_service = UserService(session)
role_service = RoleService(session)
log_permission_service = LogPermissionService(session)
...
async def read_permissions_endpoint(
    ...
    """
    try:
        accounts_with_permissions = await role_service.get_all_role_permissions(session)
        ...
        logger.error(f"HTTP Exception during permissions retrieval: {http_exc.detail}")
        raise http_exc
...
async def read_role_endpoint(
    ...
    try:
        ...
        logger.error(f"HTTP Exception during role retrieval: {http_exc.detail}")
        raise http_exc
    except Exception as e:
...
async def remove_role_permission_endpoint(
    ...
    try:
        ...
        account = await user_service.get_account_by_username(session, body.username)
        if not account:
            raise HTTPException(
        ...
        logger.error(
            f"HTTP Exception during role removal for {body.username}: {http_exc.detail}"
        )
        raise http_exc
    except Exception as e:
...
async def add_role_permission_endpoint(
    ...
    try:
        ...
        for role_id in body.added_roles or []:
            role_permission = RolePermissionCreate(user_id=account.id, role_id=role_id)
            if await role_service.create_role_permission(session, role_permission):
                await log_permission_service.create_permission_log(
                    session,
                    LogPermissionCreate(
                        user_id=str(account.id),
...
async def create_user_endpoint(
    ...
    """
    try:
        logger.info(f"Creating user: {request.username} with roles {request.role_ids}")
        ...
        if ad_attributes:
            ...
            logger.warning(f"Could not fetch AD attributes for {request.username}")
        ...
        logger.error(
            f"HTTP Exception during user creation for {request.username}: {
                http_exc.detail
            }"
        )
        raise http_exc
    except Exception as e:
        logger.error(f"Unexpected error during user creation for {request.username}.")
        logger.error(f"Details: {e}")
        logger.debug(traceback.format_exc())
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
...
async def read_domain_accounts_endpoint(
    ...
    try:
        ...
        logger.error(
            f"HTTP Exception during Domain Accounts retrieval: {http_exc.detail}"
        )
        raise http_exc
    except Exception as e:
