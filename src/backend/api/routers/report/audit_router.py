...
router = APIRouter(prefix="/report/audit", tags=["report-audit"])


# Permission Log Endpoints
@router.post(
    "/permissions",
    response_model=LogPermissionResponse,
...
async def log_permission_action(
    ...
    service = LogPermissionService(session)
    try:
        log = await service.log_permission_action(
            session,
...
@router.post(
    "/meal-requests",
    response_model=LogMealRequestLineResponse,
    status_code=status.HTTP_201_CREATED,
)
async def log_meal_request_action(
    log_create: LogMealRequestLineCreate,
    session: SessionDep,
):
    """Internal use only - Log a meal request line action."""
    service = LogMealRequestLineService(session)
    try:
        log = await service.log_meal_request_action(
            session,
...
async def query_authentication_logs(
    ...
    service = LogAuthenticationService(session)
...
async def query_meal_request_audit_logs(
    ...
    service = LogMealRequestService(session)
...
async def get_meal_request_history(
    ...
    service = LogMealRequestService(session)
    return await service.get_logs_for_meal_request(
        session, meal_request_id, page, per_page
    )
...
async def query_user_audit_logs(
    ...
    service = LogUserService(session)
...
async def get_user_audit_log(
    ...
    if not log:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"User log {log_id} not found"
        )
    return log
...
async def query_role_audit_logs(
    ...
    service = LogRoleService(session)
...
async def get_role_audit_log(
    ...
    if not log:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail=f"Role log {log_id} not found"
        )
    return log
...
async def query_configuration_audit_logs(
    ...
    service = LogConfigurationService(session)
...
async def query_replication_audit_logs(
    ...
    service = LogReplicationService(session)
...
async def manual_audit_cleanup(
    retention_days: int = Query(
        60, ge=1, le=365, description="Number of days to retain"
    ),
    session: SessionDep,
    _: dict = Depends(require_super_admin),
):
...
async def get_audit_statistics(
    ...
    try:
        ...
        total_records = sum(
            table_stats.get("total_count", 0) for table_stats in stats.values()
        )

        return {"success": True, "total_records": total_records, "tables": stats}
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
