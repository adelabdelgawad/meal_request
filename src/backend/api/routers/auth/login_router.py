...
from core.dependencies import SessionDep, CurrentUserDep, ActiveUserDep
from api.deps import get_locale
from db.database import get_application_session as get_session
...
ACCESS_TOKEN_EXPIRE_MINUTES = settings.session.access_token_minutes
REFRESH_TOKEN_EXPIRE_DAYS = settings.session.refresh_lifetime_days
...
# Services are instantiated per-request inside each endpoint function


@router.post("/login", response_model=LoginResponse)
@limiter.limit(settings.rate_limit.login)  # Brute force protection
async def login(
    request: Request,
    ...
    """
    # Instantiate services with the request session
    page_service = PageService(session)
    revoked_token_service = RevokedTokenService(session)
    session_repository = SessionRepository(session)
    user_repository = UserRepository()
    role_service = RoleService(session)
    log_auth_service = LogAuthenticationService(session)
    ...
    try:
        ...
        if account.user.is_super_admin:
            scopes = ["super_admin"]
            roles = ["super_admin"]
            logger.info(f"User '{account.user.username}' identified as super admin")
        else:
            # Fetch role names from database for regular users
            role_names = await role_service.get_user_role_names(session, user_id)
            scopes = role_names if role_names else []
            roles = role_names if role_names else []
            logger.info(f"User '{account.user.username}' has roles: {role_names}")
        ...
        if user_locale and user_locale in settings.locale.supported_locales:
            locale = user_locale
            logger.info(f"Using user's preferred locale from database: {locale}")
        else:
            logger.info(
        ...
        revoked_count = await session_repository.enforce_session_limit(
            session=session,
            user_id=user_id,
            max_sessions=settings.session.max_concurrent,
        ...
        if revoked_count > 0:
            logger.info(
                ...
                f"to enforce limit of {settings.session.max_concurrent}"
            )
        ...
        locale_max_age = settings.locale.cookie_max_age_days * 24 * 60 * 60
        response.set_cookie(
            key=settings.locale.cookie_name,
            value=locale,
            max_age=locale_max_age,
            httponly=False,
            secure=settings.locale.cookie_secure,
            samesite=settings.locale.cookie_samesite,
        )
        ...
        if account.user.is_super_admin:
            ...
            logger.info(f"Retrieved user {account.user.username} permissions: {pages}")
...
async def validate_session(
    ...
    try:
        ...
        if refresh_token:
            refresh_payload = session_helpers.verify_refresh_token(refresh_token)
    ...
    try:
        ...
        try:
            user_record = await user_repository.get_by_id(session, UUID(user_id))
            if user_record:
        ...
        try:
            if is_super_admin:
                ...
                pages_result = await page_service.get_pages_by_account(session, user_id)
                logger.info(
...
async def validate_refresh_token(
    request: Request,
    refresh_cookie: str = Cookie(None, alias=settings.session.cookie_name),
    session: SessionDep,
):
    """
    ...
    try:
        ...
        locale = payload.get("locale", settings.locale.default_locale)
...
async def refresh_token(
    ...
    try:
        ...
        try:
            ...
            token_locale = payload.get("locale", settings.locale.default_locale)
            ...
            return TokenRefreshResponse(
                access_token=access_token,
                token_type="bearer",
                expires_in=settings.session.access_token_minutes * 60,
            )
