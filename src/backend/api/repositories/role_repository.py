...
class RoleRepository:
    ...
    async def create(self, session: AsyncSession, role: Role) -> Role:
        ...
        existing = await self.get_by_name_en(session, role.name_en)
        if existing:
    ...
    async def get_by_id(self, session: AsyncSession, role_id: int) -> Optional[Role]:
        result = await session.execute(
            select(Role).where(Role.id == role_id)
        )
        return result.scalar_one_or_none()

    async def get_by_name(self, session: AsyncSession, name: str, locale: Optional[str] = None) -> Optional[Role]:
        """
    ...
    async def get_by_name_en(self, session: AsyncSession, name_en: str) -> Optional[Role]:
        """Get role by English name."""
        result = await session.execute(
            select(Role).where(Role.name_en == name_en)
        )
        return result.scalar_one_or_none()

    async def get_by_name_ar(self, session: AsyncSession, name_ar: str) -> Optional[Role]:
        """Get role by Arabic name."""
        result = await session.execute(
            select(Role).where(Role.name_ar == name_ar)
        )
        return result.scalar_one_or_none()
    ...
    async def list(
        ...
        if name_filter and name_filter.strip():
            ...
            query = query.where(
                or_(
                    Role.name_en.ilike(search_term),
                    Role.name_ar.ilike(search_term)
                )
            )
    ...
    async def update(self, session: AsyncSession, role_id: int, role_data: dict) -> Role:
