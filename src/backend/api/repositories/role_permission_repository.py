...
class RolePermissionRepository:
    ...
    async def create(
        self, session: AsyncSession, role_permission: RolePermission
    ) -> RolePermission:
    ...
    async def get_by_user_and_role(
        self, session: AsyncSession, user_id: str, role_id: int
    ) -> Optional[RolePermission]:
    ...
    async def assign_role_to_user(
        ...
        existing = await self.get_by_user_and_role(session, user_id, role_id)
        if existing:
            return existing
        ...
        return await self.create(session, role_permission)
    ...
    async def revoke_role_from_user(
        ...
        # Find the permission
        permission = await self.get_by_user_and_role(session, user_id, role_id)
        if not permission:
            raise NotFoundError(
    ...
    async def revoke_all_users_from_role(
        self, session: AsyncSession, role_id: int
    ) -> None:
    ...
    async def assign_roles_to_user(
        self, session: AsyncSession, user_id: str, role_ids: List[int]
    ) -> List[RolePermission]:
        """
        ...
        await self.revoke_all_roles_from_user(session, user_id)

        # Then, assign new roles
        permissions = []
        for role_id in role_ids:
            permission = await self.assign_role_to_user(session, user_id, role_id)
            permissions.append(permission)

        return permissions

    async def assign_users_to_role(
        self, session: AsyncSession, role_id: int, user_ids: List[str]
    ) -> List[RolePermission]:
        """
        ...
        await self.revoke_all_users_from_role(session, role_id)
    ...
    async def delete(self, session: AsyncSession, permission_id: int) -> None:
        ...
        if not permission:
            raise NotFoundError(entity="RolePermission", identifier=permission_id)
        ...
        try:
            ...
            await session.rollback()
            raise DatabaseError(f"Failed to delete role permission: {str(e)}")

    async def count_users_in_role(self, session: AsyncSession, role_id: int) -> int:
    ...
    async def count_roles_for_user(self, session: AsyncSession, user_id: str) -> int:
