...
class MealRequestStatusService:
    ...
    def __init__(self):
        self._repo = MealRequestStatusRepository()

    async def create_status(self, session: AsyncSession, name: str) -> MealRequestStatus:
        """Create a new status."""
        status = MealRequestStatus(name=name)
        return await self._repo.create(session, status)

    async def get_status(self, session: AsyncSession, status_id: int) -> MealRequestStatus:
        """Get a status by ID."""
        status = await self._repo.get_by_id(session, status_id)
        if not status:
            raise NotFoundError(entity="MealRequestStatus", identifier=status_id)
        return status

    async def list_statuses(self, session: AsyncSession, page: int = 1, per_page: int = 25) -> Tuple[List[MealRequestStatus], int]:
        """List all statuses."""
